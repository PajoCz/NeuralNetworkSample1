...
namespace NnEngineStateToImage
{
    public class LearnProgressToImage
    {
        public float PercentMissAllStart { get; set; }
        public List<float> PercentMissAll { get; set; } = new();
        public List<List<float>> PercentMissDataIndex { get; set; } = new();
        ...
        public Bitmap CreateImage(int p_Width, int p_Height)
        {
            ...
            x = (width - 2 * widthBorder) / (float)PercentMissAll.Count * -1 + widthBorder;
            y = (100 - PercentMissAllStart) / 100 * (p_Height - 2 * heightBorder) + heightBorder;
            points.Add(new Point((int)x, (int)y));
            for (var i = 0; i < PercentMissAll.Count; i++)
            {
                x = (width - 2 * widthBorder) / (float)PercentMissAll.Count * i + widthBorder;
