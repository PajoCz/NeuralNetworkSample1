...
namespace NeuralNetworkSample3_Layers
{
    public class NeuralNetworkToImage
    {
            ...
            NeuralNetworkEngine.OnTrainProgressTime p_OnTrainProgressTime)
        {
            ...
            var neuronXInc = (p_Width - 2 * widthBorder) / layers.Count;
            ...
            for (int iLayer = 0; iLayer < layers.Count; iLayer++)
            {
                ...
                {
                    var penNeuron = penNeurons[neuronPenBrushIndex];
                    var brushNeuron = brushNeurons[neuronPenBrushIndex];
                    var neuron = layer.Neurons[iNeuron];
                    var x = neuronXInc /*input synapses*/ + widthBorder + iLayer * neuronXInc;
                    ...
                    graphics.DrawString(neuron.Id, fontNeuron, brushNeuron, x - neuronSize / 2, y - neuronSize / 2);
                    graphics.DrawString($"Bias {neuron.Bias:f3}", fontNeuronBias, brushNeuron, x - neuronSize / 2, y + neuronSize / 2);
                    graphics.DrawString($"Out:{neuron.LastCalculatedOutput:f3}{Environment.NewLine}OutSig: {neuron.LastCalculatedOutputSigmoid:f3}", fontNeuronBias, brushNeuron, x - neuronSize / 2, y + neuronSize);
                    if (iLayer == layers.Count - 1 && layer.Neurons.Count == 1)
                    {
                        ...
                        {
                            graphics.DrawString($"{Environment.NewLine}Miss {p_PercentMissAll:f3}% ({p_PercentMiss:f3}%)", fontNeuronBias, brushNeuron,
                                x - neuronSize / 2, y + neuronSize / 2);
                        }
                    }
                    ...
                    {
                        var prevX = neuronXInc /*input synapses*/ + widthBorder + (iLayer-1) * neuronXInc;
                        ...
                        graphics.DrawString($"Weight {neuron.SynapsesToPreviousLayer[iPrevNeuron].Weight:f3}", fontNeuronBias, brushNeuron, prevX + (x - prevX)/3*2, prevY + (y - prevY)/3*2);
                        if (iLayer == 0 && iNeuron == 0)
                            graphics.DrawString($"In: {p_Data[iPrevNeuron]:f3}", fontNeuronBias, brushNeuron, prevX, prevY + neuronSize / 4);
                    }
