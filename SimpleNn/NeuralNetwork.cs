...
namespace SimpleNn
{
    public class NeuralNetwork
    {
        ...
        public NeuralNetwork(int[] layers, string[] layerActivations, float[] initBiases = null, float[] initWeights = null)
        {
        ...
        private void InitBiases(float[] p_InitBiases) //initializes random array for the biases being held within the network.
        {
            int iInitBias = 0;
            ...
            for (int i = 1; i < layers.Length; i++)
            {
                ...
                {
                    bias[j] = p_InitBiases != null ? p_InitBiases[iInitBias++] : random.NextSingle()-0.5f;
                }
        ...
        private void InitWeights(float[] p_InitWeights) //initializes random array for the weights being held in the network.
        {
            int iInitWeight = 0;
            ...
            for (int i = 1; i < layers.Length; i++)
            {
                ...
                for (int j = 0; j < layers[i]; j++)
                {
                    ...
                    {
                        neuronWeights[k] = p_InitWeights != null ? p_InitWeights[iInitWeight++] : random.NextSingle() - 0.5f;
                    }
        ...
        public void BackPropagate(float[] inputs, float[] expected) //backpropogation;
        {
                 ...
                 i++) //calculates the w' and b' for the last layer in the network
            {
                ...
                for (int j = 0; j < layers[layers.Length - 2]; j++)
                {
                    ...
                    weights[layers.Length - 2][i][j] -=
                        gamma[layers.Length - 1][i] * neurons[layers.Length - 2][j] * learningRate; //*learning 
                }
            }
            ...
            for (int i = layers.Length - 2; i > 0; i--) //runs on all hidden layers
            {
                ...
                for (int j = 0; j < layers[i]; j++) //itterate over outputs of layer
                {
                    ...
                    {
                        weights[i - 1][j][k] -=
                            gamma[i][j] * neurons[i - 1][k] * learningRate; //modify weights of network
                    }
                }
            }
        }
        ...
        public void Mutate(int high, float val) //used as a simple mutation function for any genetic implementations.
        {
            ...
            //    {
            //        biases[i][j] = (UnityEngine.Random.Range(0f, high) <= 2)
            ...
            //        {
            //            weights[i][j][k] = (UnityEngine.Random.Range(0f, high) <= 2)
